...
def titlee():
    return html.H1(children='COVID-19 CARE CENTER DASHBOARD',
                                        style={
                                        'textAlign': 'center',
                                        'color': colors['text']
                                    })
...
def DROPDOWN2(country_list):
    dropdown_list = []
    for country in sorted(country_list):
        tmp_dict = {'label2':country,'value2':country}
        dropdown_list.append(tmp_dict)
    return dropdown_list
...
def CIRCLETXT(card_header,card_value,overall_value):
    ...
    card_body_style = {'textAlign':'center','fontSize':'200%','fontColor':'black'}
...
def CIRCLETXT2(card_header,card_value,beds):
    card_head_style = {'textAlign':'center','fontSize':'150%','fontColor':'black'}
    card_body_style = {'textAlign':'center','fontSize':'200%','fontColor':'black'}
    card_header = dbc.CardHeader(card_header,style=card_head_style)
    card_body = dbc.CardBody(
        [
            html.H5(f"{int(card_value-200):,}", className="card-title",style=card_body_style),
        ...
        [
            dbc.Row(
                [
                              ...
                              dbc.Col(dbc.Card(CIRCLETXT2("BEDS",conf_country_total,activetotal), color="#e119f7 ", inverse=True),md=dict(size=2)),
                              
                                  dbc.Col(dbc.Card(CIRCLETXT2("OXYGEN",dead_country_total,diestotal),color="#1972f7 ", inverse=True),md=dict(size=2)),
                             dbc.Col(dbc.Card(CIRCLETXT2("OXYGEN",dead_country_total,diestotal),color=" #f76e19 ", inverse=True),md=dict(size=2)),
                              
                                  dbc.Col(dbc.Card(CIRCLETXT2("AMBULANCE AVAILABLE",dead_country_total,diestotal),color="#00edc6 ", inverse=True),md=dict(size=2)),
        ...
        [
            ...
            dbc.Row(
                [
                    ...
                    dbc.Col(get_country_dropdown(id="first-dropdown"),md=dict(size=4,offset=4))
                ]
